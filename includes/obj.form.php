...
class girafaFORM{
  ...
  function girafaFORM($title, $script_action, $table, $fieldLegend){
    ...
    if (GetParam(GetParamsCount()-2) == 'edit') {
      $sql = 'SELECT * FROM `' . $this->table . '` WHERE ID = ' . intval(base64_decode(GetParam(GetParamsCount()-1))) . ' AND Usuario = ' . $login->user_id;
      //die($sql);
...
class girafaFORM_box{
  ...
  public function AddContentLine(){
    $html = '<div style="clear:both"></div><br><div class="hr-line-dashed"></div><br>';
